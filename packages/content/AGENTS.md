...
```text
sources/* + coverage-overrides.json  →（generate-coverage）→  coverage.json
  →  ink/*.ink  →（compile-ink）→  compiled/*.json  →  manifests/*-scenes.ts（表现层）
  →  catalog/story-catalog.json  →  src/chapters/*.ts 动态加载  →  运行时 loadStoryChapter
```
...
| 来源保真      | `sources/` + `ledgers/*-coverage-overrides.json`                                | 原稿 hash、经审 adaptation receipt；生成 ledger 勿手改                       |
